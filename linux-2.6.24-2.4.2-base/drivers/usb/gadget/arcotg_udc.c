...
static void dr_phy_low_power_suspend(int phcd)
{
	u32 temp = fsl_readl(&dr_regs->portsc1);

	if ((phcd) && !(temp & PORTSCX_PHY_LOW_POWER_SPD)) {
	...
	} else if ((phcd == 0) && (temp & PORTSCX_PHY_LOW_POWER_SPD)) {
		//if (udc_controller->pdata->usb_clock_for_pm)
		...
		struct usb_ctrlrequest *setup)
{
	...
	if (setup->bRequestType & USB_DIR_IN) {
		...
		if (setup->bRequest == 0x64)
			ep0_prime_status(udc, EP_DIR_IN);
	}
