...
MODULE_PARM_DESC(debug, "Enable debugging, 0=off, 1=on, default=on");
#endif
...
static int __init gs_module_init(void)
{
	...
	//gs_tty_driver->init_termios.c_cflag = B9600 | CS8 | CREAD | HUPCL | CLOCAL; //deafult
		...
		struct usb_request *req)
{
	...
	default:
		/* unexpected */
		//aggiunta modifica per segnalare in seriale questo errore sconosiuto.
		printk(KERN_ERR "gs_setup_complete_set_line_coding: unexpected error, status=%d\n", req->status);
		break;
	}
	return;
}
