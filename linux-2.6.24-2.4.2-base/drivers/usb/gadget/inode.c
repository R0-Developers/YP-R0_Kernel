...
ep_io (struct ep_data *epdata, void *buf, unsigned len)
{
	...
	if (likely (value == 0)) {
		...
		if (value != 0) {
			printk (epdata->dev, "%s i/o interrupted start\n",	epdata->name);
			spin_lock_irq (&epdata->dev->lock);
			if (likely (epdata->ep != NULL)) {
				printk (epdata->dev, "%s i/o interrupted end\n",
						epdata->name);
...
//	spin_lock_irq (&data->dev->lock);
	if (code == GADGETFS_CANCEL_IO) {
		cancel_io_process(&status);
		printk("cancel ioctl return\n");
		return -1;
	}
//	spin_unlock_irq (&data->dev->lock);
...
/* FIXME don't call this with the spinlock held ... */
	...
	if (mtp_ctrl_cmd == 1) {
		...
		//schedule_delayed_work(&cancel_work, HZ * 3);
	}
		...
		unsigned code, unsigned long value)
{
	...
	/*20090113 kichan testing for ep0_poll() & user mode msc*/
#if 0
	switch(code)
	{
		case GADGETFS_READY_SETUP:
			if( dev->state == STATE_DEV_SETUP)
				dev->state = STATE_DEV_CONNECTED;
			return 0;
		/*linux code*/
