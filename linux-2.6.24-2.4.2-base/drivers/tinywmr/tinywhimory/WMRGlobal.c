...
WMRConfig				stConfig;
WMRLayout				stLayout;
extern 		BOOL32 					bInternalInterleaving;
...
{	
	...
	/*************************************/
	/* Whimory VFL, RO,  FTL layout information */
	/**************************************/
	
	/* RO AREA SIZE */
	#if(WMR_RO_AREA_SUPPORT)
	RO_AREA_SIZE = (WMR_RO_AREA_PB + 1 +BANKS_TOTAL - 1) / BANKS_TOTAL;    /* round up (RO_AREA_PB+1) */ 
	#else
	RO_AREA_SIZE = 0;
	#endif

	/* FTL AREA - USER DATA (Read-Write Data) SIZE */
	USER_SUBLKS_TOTAL = SUBLKS_TOTAL * WMR_USER_SUBLKS_RATIO / 256 - RO_AREA_SIZE;
	...
	RESERVED_SECTION_SIZE = SUBLKS_TOTAL - (FTL_AREA_SIZE + WMR_AREA_SIZE + VFL_INFO_SECTION_SIZE + SPECIAL_AREA_SIZE + RO_AREA_SIZE);
