...
//#define WMR_NO_NUCLEUS
#if defined(WMR_NO_NUCLEUS)
#include "nucleus.h"
#endif
...
#define     WMR_SIGNATURE       (('2' << 24) | ('2' << 16) | ('1' << 8) | ('W' << 0))
...
typedef struct {
	...
	UINT32	nRoAreaSize;				/* the size of RO Area			 */
...
#if (WMR_PROGRAM_PER_PLANE)
#define     PROGRAM_PER_PLANE           (stDeviceInfo.bUseProgramPerPlane)
#endif
...
#define		WMR_AREA_SIZE			(1)

#define     SPECIAL_AREA_START      (WMR_AREA_SIZE)
//#define     	SPECIAL_AREA_SIZE       		(129)
#define     	SPECIAL_AREA_SIZE       		(0)
...
#define		RO_AREA_START				(VFL_AREA_START+VFL_AREA_SIZE)
#define		RO_AREA_SIZE				(stLayout.nRoAreaSize)

#define		FTL_AREA_START			(RO_AREA_START + RO_AREA_SIZE)
