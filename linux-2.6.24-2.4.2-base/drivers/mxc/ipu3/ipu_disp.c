...
void _ipu_dmfc_init(void)
{
	...
	__raw_writel(0x1F1E9C9A, DMFC_DP_CHAN);
#else
		 ...
		 uint32_t out_pixel_fmt)
{
	...
	reg = __raw_readl(DP_COM_CONF(dp));
	if ( !color_key_4yuv && (reg & DP_COM_CONF_GWCKE) &&
		...
		printk("_ipu_dp_init color key 0x%x need change to yuv fmt!\n", color_key);
		...
		printk("_ipu_dp_init color key change to yuv fmt 0x%x!\n", color_key);
	}
			    ...
			    uint32_t v_to_h_sync, ipu_di_signal_cfg_t sig)
{
	...
	div &= 0xFF8;
#else
	/* Get the closes integer divider */
...
#ifdef DVFS_IPU_WORKAROUND
		_ipu_di_sync_config(disp, 6, 1, 0,
				2, DI_SYNC_CLK,
				((width + h_start_width + h_sync_width) / 2) - 2,
...
#ifdef DVFS_IPU_WORKAROUND
		di_gen |= 0x6000000; /* Set the clock to stop at counter 6. */
#endif
	}
			       ...
			       uint32_t color_key)
{
	...
	if (((fg_csc_type == RGB2YUV) && (bg_csc_type == YUV2YUV)) ||
		...
		printk("color key 0x%x need change to yuv fmt\n", color_key);
		...
		printk("color key change to yuv fmt 0x%x\n", color_key);
	}
