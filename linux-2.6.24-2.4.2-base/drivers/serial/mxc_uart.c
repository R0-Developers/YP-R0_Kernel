...
extern int gpio_pba_is_active(void);
...
static ssize_t set_lp_mode(struct device *dev, struct device_attribute *attr, 
			   char *buf, size_t size)
{
	struct platform_device *pdev = to_platform_device(dev);
	uart_mxc_port *umxc = platform_get_drvdata(pdev);
	pm_message_t event = PMSG_SUSPEND;
	...
	if (umxc->suspend) {
		...
		if (!umxc->port.info ||
		    !(umxc->port.info->flags & UIF_INITIALIZED))
		return size;
		if (!strncasecmp(buf, "enter", 5)) {
			umxc->suspend = 1;
			mxcuart_suspend(pdev, event);
			suspend_console();
		}
	}
	return size;
...
static DEVICE_ATTR(lp_mode, 0222, NULL, set_lp_mode);
...
static int mxcuart_probe(struct platform_device *pdev)
{
	...
	if (mxc_ports[id]->enabled == 1) {
		...
		if (mxc_ports[id]->clk == NULL)
			return -1;
		if (device_create_file(&pdev->dev, &dev_attr_lp_mode)) {
			printk(KERN_ERR
			       "unable to register sysdev entry for lp_mode\n");
			return -1;
		}
