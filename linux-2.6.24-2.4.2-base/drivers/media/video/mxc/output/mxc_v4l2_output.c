...
extern void ipu_wait_interrupt(int irq, int ms);
...
static void timer_work_func(struct work_struct *work)
{
	...
	atomic_set(&vout->in_timer_work, 1);

	ipu_wait_interrupt(vout->disp_irq, 60);

	spin_lock_irqsave(&g_lock, lock_flags);

	/* the second buffer ready */
	if (vout->frame_count == 1) {
...
exit0:
	...
	atomic_set(&vout->in_timer_work, 0);
}
...
static void mxc_v4l2out_timer_handler(unsigned long arg)
{
	...
	if (vout->ipu_buf[vout->next_rdy_ipu_buf] != -1) {
		dev_err(vout->video_dev->dev, "IPU buffer busy\n");
		if (atomic_read(&vout->in_timer_work) == 0)
			vout->state = STATE_STREAM_PAUSED;
		goto exit0;
	}
...
static irqreturn_t mxc_v4l2out_pp_in_irq_handler(int irq, void *dev_id)
{
	...
	/* the first frame arrive? */
	if (g_irq_cnt == 1) {
		if (queue_work(vout->v4l_wq, &vout->timer_work) == 0)
			dev_err(vout->video_dev->dev, "first work was in queue already!\n ");
...
static int mxc_v4l2out_streamon(vout_data * vout)
{
	...
	switch (vout->display_ch) {
		case MEM_FG_SYNC:
			vout->disp_irq = IPU_IRQ_FG_SYNC_EOF;
			break;
		case MEM_BG_SYNC:
			vout->disp_irq = IPU_IRQ_BG_SYNC_EOF;
			break;
		case MEM_DC_SYNC:
